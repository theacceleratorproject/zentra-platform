      ...
      <div className="zen-card p-5">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-semibold">Fee Engine</h3>
              <p className="text-xs text-slate-500 font-mono">FEE-ENGINE.cbl — maintenance, low-balance, overdraft fees</p>
            </div>
          </div>
          {!feesData && (
            <button onClick={async () => { setFeesLoading(true); setFeesData(await api.runFees()); setFeesLoading(false); }} disabled={feesLoading} className="flex items-center gap-2 px-4 py-2 bg-gold-500 text-primary-foreground font-semibold rounded-lg text-sm hover:opacity-90 disabled:opacity-50">
              {feesLoading ? <Loader2 size={14} className="animate-spin" /> : <Play size={14} />} Run
            </button>
          )}
        </div>
        {feesData && (
          <div className="grid grid-cols-4 gap-3 mt-4">
            <div className="bg-navy-700/50 rounded-lg p-3 text-center"><p className="text-xs text-slate-500">Maintenance</p><p className="font-mono text-lg font-semibold">{feesData.maintenance}</p></div>
            <div className="bg-navy-700/50 rounded-lg p-3 text-center"><p className="text-xs text-slate-500">Low Balance</p><p className="font-mono text-lg font-semibold">{feesData.low_balance}</p></div>
            <div className="bg-navy-700/50 rounded-lg p-3 text-center"><p className="text-xs text-slate-500">Overdraft</p><p className="font-mono text-lg font-semibold">{feesData.overdraft}</p></div>
            <div className="bg-navy-700/50 rounded-lg p-3 text-center"><p className="text-xs text-slate-500">Total Fees</p><p className="font-mono text-lg font-semibold text-gold">{formatCurrency(feesData.total)}</p></div>
          </div>
        )}
      </div>
      ...
      <div className="zen-card p-5">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-semibold">Interest Calculator</h3>
              <p className="text-xs text-slate-500 font-mono">INTEREST-CALC.cbl — daily interest from rate table</p>
            </div>
          </div>
          {!intData && (
            <button onClick={async () => { setIntLoading(true); setIntData(await api.runInterest()); setIntLoading(false); }} disabled={intLoading} className="flex items-center gap-2 px-4 py-2 bg-gold-500 text-primary-foreground font-semibold rounded-lg text-sm hover:opacity-90 disabled:opacity-50">
              {intLoading ? <Loader2 size={14} className="animate-spin" /> : <Play size={14} />} Run
            </button>
          )}
        </div>
        {intData && (
          <div className="grid grid-cols-2 gap-3 mt-4">
            <div className="bg-navy-700/50 rounded-lg p-3 text-center"><p className="text-xs text-slate-500">Accounts Credited</p><p className="font-mono text-lg font-semibold">{intData.accounts_credited}</p></div>
            <div className="bg-navy-700/50 rounded-lg p-3 text-center"><p className="text-xs text-slate-500">Total Interest</p><p className="font-mono text-lg font-semibold text-zen-green">{formatCurrency(intData.total_interest)}</p></div>
          </div>
        )}
      </div>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 font-mono">EOD-REPORT.cbl — management summary</p>
            </div>
          </div>
          {!eodData && (
            <button onClick={async () => { setEodLoading(true); setEodData(await api.getEOD()); setEodLoading(false); }} disabled={eodLoading} className="flex items-center gap-2 px-4 py-2 bg-gold-500 text-primary-foreground font-semibold rounded-lg text-sm hover:opacity-90 disabled:opacity-50">
              {eodLoading ? <Loader2 size={14} className="animate-spin" /> : <Play size={14} />} Generate Report
            </button>
          )}
        </div>
        {eodData && (
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          {!files && (
            <button onClick={async () => { setFilesLoading(true); setFiles((await api.getFiles()).files); setFilesLoading(false); }} disabled={filesLoading} className="flex items-center gap-2 px-4 py-2 border border-border text-foreground font-medium rounded-lg text-sm hover:border-primary/30 hover:text-gold transition-colors disabled:opacity-50">
              {filesLoading ? <Loader2 size={14} className="animate-spin" /> : null} Scan Files
            </button>
          )}
        </div>
        {files && (
